"""layer2/emit/instructions/consumer_binding/ — assemble the consumer-driving params the DATA-fill helper passes to V48's
...
re-exported here so `from layer2.emit.instructions.consumer_binding import build, page_endpoint, canonical_screen,
...
variants, the page→endpoint map) is read from `endpoint_registry`, which carries the (legacy-EMS-derived) `_PAGES` route table.
