"""layer2/emit/data/consumer_binding/screen_map.py — map a FE page_key / a card's backend_strategy to its ems_backend
...
ems_backend's OWN `_PAGES` route table, so this follows ems_backend automatically (no drift)."""
from layer2.emit.data.endpoint_registry import PAGE_PRIMARY
from config.app_config import cfg

# FRONTEND page-tail → ems_backend page CODE, ONLY where the names differ (a frontend naming convention, NOT endpoint
...
def page_endpoint(page_key):
    """FE page_key → its LIVE ems_backend endpoint (derived from PAGE_PRIMARY). Back-compat page-level live_frame."""
...
def canonical_screen(backend_strategy):
    ...
    canonical ems_backend screen, SHOWN TO THE AI as a hint (the AI still decides the endpoint, incl. a history variant)."""
