"""lib/parallel.py — the one concurrency primitive: run thunks in threads, join, preserve order/exceptions. [orchestrator]

TRACE PROPAGATION [obs]: each thunk runs under a COPY of the caller's contextvars context, so the active
observability trace/span (obs/trace.py) follows every fan-out (1a∥1b, per-card L2 emits) into its worker thread —
per-card spans nest under the span that was active when the pool was entered. One copy per thunk (a Context object
cannot be entered concurrently).

Home moved from run/parallel.py (sys.modules-alias facade kept there) — grounding/validate/layer1b consume
...
def run_parallel(named_thunks, max_workers=None):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futs = {name: ex.submit(contextvars.copy_context().run, fn) for name, fn in named_thunks.items()}
