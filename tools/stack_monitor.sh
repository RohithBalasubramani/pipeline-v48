...
# Owned services it keeps alive: host :8770, frontend :5188, ems_backend :8890. External (vLLM :8200, neuract :5433)
...
start_ems() { setsid bash -c "cd $ROOT/../pipeline_v45/ems_backend && exec python3 manage.py runserver 0.0.0.0:8890 --noreload" > $ROOT/outputs/logs/ems_backend.log 2>&1 < /dev/null & disown; }
...
while [ ! -f /tmp/stack_monitor.stop ]; do
  ...
  if up 8890; then status+="ems:UP ";  else status+="ems:DOWN->restart ";  start_ems; fi
