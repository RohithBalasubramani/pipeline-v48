...
// Card 69 — Current History (page diesel-generator-asset-dashboard/voltage-current, CMD V2 DG "Voltage & Current" tab).
// HistoryPanel fed its OWN Layer-2 payload's current `data` (HistoryPanelData).
//
...
// payload with no usable HistoryPanelData falls back to CMD V2's OWN structured-empty slice — NEVER a blank/null card,
// NEVER a mock/seed number. Its header SamplingPicker drives a per-card re-fetch via onDateChange.
...
function CurrentHistoryCard({ payload, onDateChange }: { payload: any; onDateChange?: OnDateChange }) {
  const fromPayload = historyData(payload);
  // ALWAYS-DRAW: prefer the payload's HistoryPanelData; if unusable, fall back to CMD V2's OWN structured-empty slice
  // (series [], stats '—') so the panel draws its chrome instead of a blank/null card.
  const usable = (d: any) => !!d && Array.isArray(d.series);
  const data = usable(fromPayload) ? fromPayload! : unavailableHistory("current");
...
export const card69 = (p: any, _f?: any, od?: OnDateChange): React.ReactNode => (
