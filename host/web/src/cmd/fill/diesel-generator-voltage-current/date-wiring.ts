// date control wiring (HistoryPanel SamplingPicker → host window) for the diesel-generator
...
// onDateChange, which re-fetches JUST this card's ems_backend frame. The shared
// electrical hook (useVoltageCurrentData.voltageCurrentHistorySelectionToParams)
// is the reference for the preset→sampling pairing; the host range tokens are the
 ...
 *  resolution the card's history grid uses — `2hour`, matching the detail tabs'
 *  today grid). */
