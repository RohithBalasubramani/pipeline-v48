"""Shared range × sampling time-filter vocabulary for aggregate dispatchers.

The frontend's "Show Events for" / "Energy Consumption Trend" dropdowns speak
one vocabulary; the SQL helpers speak another. This module is the single
place that:

  * accepts the frontend's vocab (``last-7-days``, ``daily``, ``custom-range``)
    plus older internal aliases (``last_7d``, ``day``);
  * validates ``range × sampling`` combos;
  * resolves a window + builds chronological bucket edges anchored to IST;
  * labels buckets per the design system (``D-N`` / ``Today`` / ``A`` / ``B`` /
    ``W-N`` / ``This W`` / day-of-month).

Both the Voltage & Current and Energy & Power PCC dispatchers import from here
so the two pages can never drift in how a preset resolves.
"""
...
# ── Range × sampling matrix (frontend vocabulary) ─────────────────────────
# `hourly` = 3-hour buckets (8 per day) — kept for back-compat; legacy name.
# `2hour`  = 2-hour buckets (12 per day) — added for the detail-tab Power
#           Energy Analysis chart which expects 12 bars over a 24 h range.
SAMPLING_BY_RANGE: dict[str, tuple[str, ...]] = {
    'today':        ('hourly', '2hour', 'shift'),
    'yesterday':    ('hourly', '2hour', 'shift'),
    'last-7-days':  ('daily',),
    'last-30-days': ('daily', 'weekly'),
    'this-month':   ('daily', 'weekly'),
    'last-month':   ('daily', 'weekly'),
    'custom-range': ('hourly', '2hour', 'shift', 'daily', 'weekly'),
}
...
def validate_range_sampling(preset: str, sampling: str) -> str | None:
    """Return None if the combo is legal; else a precise error message."""
    if preset not in SAMPLING_BY_RANGE:
        return (f"range='{preset}' not supported. Allowed: "
                f"{', '.join(sorted(SAMPLING_BY_RANGE))}")
    if sampling not in BUCKET_SECONDS_BY_SAMPLING:
        return f"sampling='{sampling}' not supported. Allowed: hourly, 2hour, shift, daily, weekly"
...
def make_bucket_labels(bucket_starts: list[datetime], sampling: str,
    ...
    """Per-bucket display label, matching the frontend design system.

    hourly → "HH:MM" · shift → "A"/"B"/"C" ·
    daily  → "D-N"/"Today" (today-anchored) or "DD" (month-scope) ·
    weekly → "W-N"/"This W" (this-month) or "W-N" (last-month).
    """
    local_starts = [ts.astimezone(LOCAL_TZ) for ts in bucket_starts]

    if sampling in ('hourly', '2hour'):
...
def build_bucket_edges(preset: str, sampling: str,
    ...
    """Resolve (preset, sampling) → chronological [(start, end, label)].

    Bucket starts anchor to IST midnight (or the 1st of the month for
    month-scope weekly) so labels line up with shifts / calendar days.
    ``custom_start`` / ``custom_end`` are required when preset is
    ``custom-range``. ``timeline_time`` caps the right edge for today/yesterday.
    """
