"""domain/quantity_class.py — the PHYSICAL-QUANTITY class vocabulary + compatibility test + const-source resolver.
...
seeded by db/seed_quantity_class.sql; the code defaults below behave identically until a row exists. Home
moved from layer2/quantity_class.py (facade kept there) — ems_exec.measurable_resolve consumes the same vocabulary and
the back-import made layer2↔ems_exec circular. [cycle-kill 2026-07-12] Generic — NO
