"""domain/asset_3d.py — exact_metadata for a 3D-viewer card: the resolved GLB object + viewer context.
...
Home moved from layer2/emit/metadata/asset_3d.py (facade kept there) — the ems_exec 3D renderer consumes
this resolver directly and the back-import made layer2↔ems_exec circular. [cycle-kill 2026-07-12]
"""
import os
...
def _tier_kitpreview(asset_table, page_type):
    """Tier 5 — the LOCAL equipment kit-preview fallback (stream D). ``(object, gap_cause)``:
      ...
      · DEFAULT-DENY existence gate: the model's glb_file is registry METADATA — the object ships ONLY when the
        ...
        config.asset3d_media.glb_url (the web origin's /media/ route), never the checked filesystem path;
...
def _kitpreview_local_glb(glb_file):
    ...
    knob equipment.kitpreview.media_base — BY CONTRACT a LOCAL filesystem directory (the same dir the media
